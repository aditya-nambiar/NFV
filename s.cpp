...
void* multithreading_func(void *arg){
	ClientDetails client = *(ClientDetails*)arg;
	RawServer server;
	Packet pkt;
	server.fill_server_details(5001, "127.0.0.1");
	server.bind_server();
	server.connect_with_client(pkt);
	print("Connection successful with client");
	return NULL;
}
...
int main(){
	RawServer server;
	Packet pkt;
	RawServer::set_interface("lo");
	server.fill_server_details(5000, "127.0.0.1");
	server.bind_server();
	server.listen_accept(multithreading_func, pkt);
	return 0;
}
